---
title: "7_Regression_Efficiency_with_wr_Environmentals"
...
```
## chunk 1:  Linear Regression model: efficiency with weight restrictions ~ environmental variables (identification of costdrivers/location factors)
```{r}
...
```
## chunk 4: OLS model: efficiency with weight restrictions ~ environmental variables  with feature selection 
```{r}
require(robustbase)
require(car)
OLS <- lm (formula= mydata_balanced$Efficiency.with.Weight.restrictions ~ mydata_balanced$Age.of.assets + mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Average.declivity + mydata_balanced$Intersection.with.indigenous.areas + mydata_balanced$Height.of.vegetation + mydata_balanced$Percentage.of.high.vegetation + mydata_balanced$Average.precipitation + mydata_balanced$Network.density + mydata_balanced$Fire.incidence + mydata_balanced$Number.of.intersections.with.roads + mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Density.of.lightening + mydata_balanced$Number.of.access.to.roads, data = mydata_balanced)
...
OLS_adj <- lm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~     mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
         mydata_balanced$Percentage.of.high.vegetation + 
        mydata_balanced$Network.density + mydata_balanced$Fire.incidence + 
        mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced)
...
# Train the model
 model <- train(x = mydata_balanced[,c(14, 16, 18, 20, 21, 22, 23, 25)], y = mydata_balanced[,26], method = "lm", trControl = train.control)
...
```
## chunk 6: Pooled OLS model: efficiency with weight restrictions ~ environmental variables after feature selection 

```{r}
require(plm)

OLS <- plm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
         mydata_balanced$Percentage.of.high.vegetation + 
        mydata_balanced$Network.density + mydata_balanced$Fire.incidence + 
        mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced, model = "pooling", index = NULL)
...
dwtest(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
        mydata_balanced$Percentage.of.high.vegetation + mydata_balanced$Network.density + 
        mydata_balanced$Fire.incidence + mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced)

```
## chunk 8: Fixed Effect Model (FE): efficiency with weigth restrictions after feature selection 
```{r}
...
FE_individual <- plm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
        mydata_balanced$Percentage.of.high.vegetation + mydata_balanced$Network.density + 
        mydata_balanced$Fire.incidence + mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced, model = "within", index = NULL, effect = "individual")
...
FE_time <- plm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
        mydata_balanced$Percentage.of.high.vegetation + mydata_balanced$Network.density + 
        mydata_balanced$Fire.incidence + mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced, model = "within", index = NULL, effect = "time")
...
FE_both <- plm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
        mydata_balanced$Percentage.of.high.vegetation + mydata_balanced$Network.density + 
        mydata_balanced$Fire.incidence + mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced, model = "within", index = NULL, effect = "twoways")
...
print(mse(mydata_balanced$Efficiency.with.Weight.restrictions, yhat_individual))
print("MSE time")
print(mse(mydata_balanced$Efficiency.with.Weight.restrictions, yhat_time))
print("MSE both")
print(mse(mydata_balanced$Efficiency.with.Weight.restrictions, yhat_both))

#compute RMSE

print("RMSE individual")
print(rmse(mydata_balanced$Efficiency.with.Weight.restrictions, yhat_individual))
print("RMSE time")
print(rmse(mydata_balanced$Efficiency.with.Weight.restrictions, yhat_time))
print("RMSE both")
print(rmse(mydata_balanced$Efficiency.with.Weight.restrictions, yhat_both))
...
RE_individual <- plm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
         mydata_balanced$Percentage.of.high.vegetation + 
        mydata_balanced$Network.density + mydata_balanced$Fire.incidence + 
        mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced, model = "random", index = NULL, effect = "individual")

summary(RE_individual)

# RE with  time effects 

RE_time <- plm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
         mydata_balanced$Percentage.of.high.vegetation + 
        mydata_balanced$Network.density + mydata_balanced$Fire.incidence + 
        mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced, model = "random", index = NULL, effect = "time", random.method = "amemiya")

summary(RE_time)

# RE with  individual and time effects 

RE_both <- plm(formula = mydata_balanced$Efficiency.with.Weight.restrictions ~ 
    mydata_balanced$`(Age.of.assets).square` + mydata_balanced$Intersection.with.indigenous.areas + 
       mydata_balanced$Percentage.of.high.vegetation + 
        mydata_balanced$Network.density + mydata_balanced$Fire.incidence + 
        mydata_balanced$Number.of.intersections.with.roads + 
        mydata_balanced$Coincidence.of.network.lines + mydata_balanced$Number.of.access.to.roads, 
    data = mydata_balanced, model = "random", index = NULL, effect = "twoways", random.method = "amemiya")
