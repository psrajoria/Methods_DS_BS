---
title: "8_Poly_Regression_Efficiency_without_wr_Environmentals"
...
```
## chunk 1: Polynomial Regression model: efficiency without weight restrictions ~ environmental variables (identification of costdrivers/location factors)
```{r}
...
```
## chunk 3:  polynomial regression model: efficiency without weight restrictions ~ environmental variables 
...
filename <- paste("residualplot_", degree,".png", sep="")
...
dwtest(formula = mydata_balanced$Efficiency.without.Weight.restrictions ~ 
    poly(mydata_balanced$Age.of.assets, degree) + poly(mydata_balanced$Intersection.with.indigenous.areas, 
        degree) + poly(mydata_balanced$Height.of.vegetation, 
        degree) + poly(mydata_balanced$Percentage.of.high.vegetation, 
        degree) + poly(mydata_balanced$Average.precipitation, 
        degree) + poly(mydata_balanced$Fire.incidence, degree), 
...
# Train the model
 model <- train(x = data.frame(poly(mydata_balanced[, 13], degree), poly(mydata_balanced[, 16], degree), poly(mydata_balanced[,17],degree), poly(mydata_balanced[, 18],degree), poly(mydata_balanced[, 19],degree), poly(mydata_balanced[, 21], degree)), y= mydata_balanced[,27], method = "lm", trControl = train.control)
...
# Train the model
 model <- train(x = data.frame(poly(mydata_balanced[, 13], degree), poly(mydata_balanced[, 15], degree), poly(mydata_balanced[,17],degree), poly(mydata_balanced[, 18],degree)), y= mydata_balanced[,27], method = "lm", trControl = train.control)
