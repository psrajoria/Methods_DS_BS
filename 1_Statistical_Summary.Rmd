...
# Data preparation
library("tidyverse")
df <-  mydata %>% select(year, company,OPEX, Efficiency.with.Weight.restrictions, Efficiency.without.Weight.restrictions) %>% slice(1:4, 7:10, 13:16, 19:22, 25:28, 31:34, 37:40, 43:46, 49:52, 55:74)
yearly_counts <- df %>% count(year, company, OPEX, Efficiency.with.Weight.restrictions, Efficiency.without.Weight.restrictions)
...
ggplot(data = yearly_counts, aes(x = year, y = Efficiency.with.Weight.restrictions, color = company)) +
  geom_line(size = 1.3) + scale_y_continuous(labels = scales::comma) +
  ylab("Efficiency with Weight restrictions") +
...
# Visualization of Efficiency.without.Weight.restrictions of all companies over the years
ggplot(data = yearly_counts, aes(x = year, y = Efficiency.without.Weight.restrictions, color = company)) +
  geom_line(size = 1.0) + scale_y_continuous(labels = scales::comma) +
  ylab("Efficiency without Weight restrictions") +
  xlab("Year") + theme(axis.title.y = element_text(size = 9, angle = 90, face = "bold")) +
  theme(axis.title.y = element_text(size = 15, angle = 90, face = "bold")) +
  theme(axis.title.x = element_text(size = 15, face = "bold")) +
  theme(legend.title = element_text(size = 15, face="bold")) +
  ...
  #       legend.key.height = unit(1, 'cm'), #change legend key height
  #       legend.key.width = unit(1, 'cm'), #change legend key width
  #       legend.title = element_text(face = "bold", size=6), #change legend title font size
  #       legend.text = element_text(size=9))
...
# dev.off()
filename_Histo <- paste(colnames(df_2[i]), "SUMMARY",".png", sep = "")
...
library("egg")
df_2 <- mydata %>%
  dplyr:: select(company:Efficiency.without.Weight.restrictions) %>% slice(1:4, 7:10, 13:16, 19:22, 25:28, 31:34, 37:40, 43:46, 49:52, 55:74)
yearly_counts_2 <- df_2 %>% count(OPEX, company, year)
df_2
...
df_3 <- mydata %>%
 dplyr:: select(company:Efficiency.without.Weight.restrictions) %>% slice(1:4, 7:10, 13:16, 19:22, 25:28, 31:34, 37:40, 43:46, 49:52, 55:74)
yearly_counts_3 <- df_3 %>% count(OPEX, company, year, Efficiency.with.Weight.restrictions, Efficiency.without.Weight.restrictions)
df_3
